REM ============================================================
REM WARNING: This is the dangerous v1.0.0 script.
REM It deletes system files (IME/Cursors/Help/Windows Update),
REM disables services, and contains hardcoded personal paths.
...
ECHO		Building: %build%
ECHO	�?					�?
ECHO.	支持 Windows XP/7/8/10
ECHO.
ECHO.	我的 Windows 版本�?VER

ECHO.
ECHO.	现在弢�始清理优�?Windows�?ECHO.	建议您���出所有正在运行的软件�?ECHO.	以达到最好的清理效果�?ECHO.	请按右键“以管理员身份运行��本程序！！�?  
ECHO.
...
REM 无用文件夹清�?DEL "%LOCALAPPDATA%\Microsoft\Windows\Cache\*.*"  /Q  /S
...
ECHO	清理 Prefetch 文件�?DEL "%WINDIR%\Prefetch\*.*"  /Q /f


ECHO	清理 Windows7 启动�?DEL "%USERPROFILE%\Start Menu\*.com*"  /Q  /S
...
DEL "%APPDATA%\Microsoft\Internet Explorer\Quick Launch\因特网搜�?lnk"

ECHO.
ECHO	清理IE收藏夹原始条�?DEL "%SYSTEMDRIVE%\Documents and Settings\Default User\Favorites\MSN.com.url"
...
DEL "%SYSTEMDRIVE%\Documents and Settings\Default User\Favorites\链接\自定义链�?url"
...
DEL "%USERPROFILE%\Favorites\链接\自定义链�?url"
...
ECHO	清理WIN7 IE收藏夹原始条�?RD  "%USERPROFILE%\Favorites\MSN 网站"  /Q  /S
...
DEL "%ALLUSERSPROFILE%\「开始��菜单\程序\紫光拼音输入�?3.0\因特网搜�?lnk"
DEL "%USERPROFILE%\「开始��菜单\程序\紫光拼音输入�?3.0\因特网搜�?lnk"
...
ECHO	清理 ProgramData 文件�?RD  "%ProgramData%\Axure\"  /Q  /S
...
ECHO.
ECHO	清理 ACDSee 的插�?图标，字�?DEL "%ProgramFiles% (x86)\ACD Systems\ACDSee\9.0\*.chm"  /Q  /S
